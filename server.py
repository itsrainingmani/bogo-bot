...
def handle():
    ...
    ##       DEV RESPONSES          ##
            ...
            response = utils.queue_user(supabase_client, sender_id, times=time_pref)
            updated_times = [TIME_OPTIONS[ch] for ch in response]
            content_response = f"Your time interval preferences were updated: {', '.join(updated_times)}\n"
            return {"content": content_response + FOOD_PREF_COMMANDS}

        elif content[:4] == 'pref':
            food_pref = content[4:].split()
            response = utils.queue_user(supabase_client, sender_id, prefs=food_pref)
            updated_prefs = [FOOD_OPTIONS[i].lower() for i in response]
            content_response = f"Your cuisine preferences were updated: {', '.join(updated_prefs)}\n"
